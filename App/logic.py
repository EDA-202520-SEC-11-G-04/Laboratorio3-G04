...
def get_best_book(catalog):
    ...
    current = catalog['books']['elements'] 
    if lt.size(catalog['books']) > 0:
        best_book = current[0]
        for book in current:
            if float(book['average_rating']) > float(best_book['average_rating']):
                best_book = book
    end_time = getTime()
    tiempo_transcurrido = deltaTime(end_time, start_time)
...
def count_books_by_tag(catalog, tag):
    ...
    for bt in catalog['book_tags']['elements']:
        # buscamos el nombre del tag correspondiente
        pos = lt.is_present(catalog['tags'], tag, compare_tag_names)
        if pos > 0:
            tag_elem = lt.get_element(catalog['tags'], pos)
            if bt['tag_id'] == tag_elem['tag_id']:
                resultado += 1
    end_time = getTime()
    tiempo_transcurrido = deltaTime(end_time, start_time)
